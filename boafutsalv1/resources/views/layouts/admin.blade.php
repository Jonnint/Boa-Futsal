    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 w-72 bg-black/40 backdrop-blur-2xl border-r border-white/5 z-40 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto sidebar-scroll py-6 px-4 space-y-2 mt-16 lg:mt-0">
            ...
            <div class="pt-6 pb-2">
                ...
                <a href="{{ route('admin.sport-types.edit-active') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->is('admin/sport-types/edit-active') || request()->is('admin/sport-types/*/edit') ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    <span class="font-bold text-sm">Kelola Konten Sport</span>
                </a>
            </div>

            @if(Auth::user()->isDeveloper())
            <div class="pt-6 pb-2">
                <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Developer Tools</p>
                <a href="/admin/sport-types" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->is('admin/sport-types*') ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
                    <span class="font-bold text-sm">Switch Sport Type</span>
                    <span class="ml-auto px-2 py-0.5 bg-blue-500/20 border border-blue-500/30 text-blue-400 text-[10px] font-extrabold rounded-md uppercase">DEV</span>
                </a>
            </div>
            @endif
        ...
        <div class="p-4 border-t border-white/5 shrink-0">
            <div class="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-400 truncate">{{ Auth::user()->isDeveloper() ? 'Developer' : 'Administrator' }}</p>
                </div>
            </div>
